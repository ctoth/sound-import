...
class Sound(object):
  ...
  def __init__(self, in_file, out_dir="."):
    ...
    self.metadata_file = os.path.join(out_dir, self.filename + ".json")
  ...
  def convert_to_intermediate(self):
    ffmpeg.input(self.in_file) \
      .audio \
      .output(self.intermediate_filename, format="wav", acodec='pcm_s24le') \
      .overwrite_output() \
      .run()
  ...
  def render_output(self):
    ffmpeg.input(self.output_wav, format="wav") \
      .audio \
      .output(self.out_file, audio_bitrate=self.OUTPUT_BITRATE) \
      .overwrite_output() \
      .run()
    metadata = {
      'duration': self.metadata['format']['duration'],
      'original_bitrate': self.metadata['format']['bit_rate'],
      'original_format': self.metadata['format']['format_name'],
      'channels': self.metadata['streams'][0]['channels'],
      'tags': self.metadata['streams'][0]['tags'],
      'sample_rate': self.metadata['streams'][0]['sample_rate'],


    }
    with open(self.metadata_file, 'wt') as f:
      json.dump(metadata, f, skipkeys=['streams'], indent=2)
